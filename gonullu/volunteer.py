...
class Volunteer(Docker):
    ...
    def get_package_farm(self, response):
        self.package = response['package_name']
        ...
        self.add_volume('/tmp/gonullu/%s' % self.package, '/root')
        ...
        self.preparation(self.kernel_requirement, self.cpu_set)
        self.set_command('/build/build.sh', self.queue_id, self.commit_id, self.package)
        self.start()
...
""" % (krn, j)
        ...
        f = open(os.path.join(build_directory, 'build.sh'), 'w')
        f.write(build_sh)
        f.close()
        os.chmod(os.path.join(build_directory, 'build.sh'), 0o755)
